...
class MainWindow(QWidget):
    ...
    def _update_table(self, table_gbox):
        sel_zap = "SELECT * FROM timetable.timetable WHERE day='{}' ORDER BY num_week".format(table_gbox)
    ...
    def _change_day_from_table(self, button):
        ...
        try:
            ...
            sel_zap = "UPDATE timetable.timetable SET subject='{0}', num_room='{1}', start_time='{2}', num_week='{3}' WHERE day='{4}' and subject='{5}' and num_room='{6}' and start_time='{7}' and num_week='{8}'".format(
    ...
    def _insert_row_table(self):
        try:
            ...
            sel_zap = "INSERT INTO timetable.timetable (id, day, subject, num_room, start_time, num_week) VALUES ('{}','{}','{}','{}','{}','{}')".format(*input().split(', '))
            self.cursor.execute(sel_zap)
            self.conn.commit()
            ...
            if box[1] == num_str[1]:
                ...
                for i in range(self.table.columnCount()):
                    try:
                        ...
                        row.append(None)
        try:
            sel_zap="DELETE FROM timetable.timetable WHERE day='{}' and subject='{}' and num_room='{}' and start_time='{}' and num_week='{}'".format(num_str[1], row[0], row[1], row[2],row[3])
    ...
    def _create_teacher_tab(self):
        ...
        self.cursor.execute("SELECT * FROM timetable.teacher")
    ...
    def _create_subjects_tab(self):
        ...
        self.cursor.execute("SELECT * FROM timetable.subject")
        records = self.cursor.fetchall()
